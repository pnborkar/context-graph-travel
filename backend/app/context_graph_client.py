...
class CypherResultCollector:
    ...
    def emit_rag_comparison(self, data: dict) -> None:
        """Emit a rag_comparison SSE event with vector vs graph expansion results."""
        self._push_event("rag_comparison", data)
...
async def execute_cypher(
    ...
    *,
    ...
    try:
        ...
        # Always emit tool_end so the frontend never hangs on "running"
        if tool_name and collect:
            _collector.collect_tool_call(tool_name, parameters or {}, "")
        raise
