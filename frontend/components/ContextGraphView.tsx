...
function NvlGraph({
  ...
  // Auto-fit when schema or new data loads
  useEffect(() => {
    if (isReady && fitOnLoad && nvlRef.current) {
      const timer = setTimeout(() => {
        try {
          nvlRef.current?.fit?.();
        } catch { /* NVL may not expose fit on all versions */ }
      }, 200);
      return () => clearTimeout(timer);
    }
  }, [isReady, fitOnLoad, nodes.length]);
